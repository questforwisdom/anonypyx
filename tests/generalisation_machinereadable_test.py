...
def test_query_overlap_categorical(mixed_schema):
    query = {'QI2': {'A', 'B'}}
    record = pd.Series({'QI1_min': -1, 'QI1_max': 3, 'QI2_A': True, 'QI2_B': True, 'QI2_C': False, 'S': 4, 'count': 1})
...
def test_query_overlap_count(mixed_schema):
    query = {'QI1': (-1, 1), 'QI2': {'A', 'B'}, 'S': (3,4)}
    record = pd.Series({'QI1_min': -1, 'QI1_max': 3, 'QI2_A': True, 'QI2_B': True, 'QI2_C': False, 'S': 4, 'count': 5})
    assert 6 == mixed_schema.query_overlap(record, query)
